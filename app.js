...
(function () {
  ...
  function semAugment(out, titleOnly, groups, searchableGroups, raw, matchedSet, ignoredFilterGroup) {
    ...
    semScan(semQ, 100).then(function (hits) {
      ...
      pass.forEach(function (s) {
        if (!hmap[s.id]) hmap[s.id] = { id: s.id, convUuid: s.convUuid, score: s.score, date: s.date, sem: true };
      });
      var out2 = [];
      ...
      renderSearchResults(out2, titleOnly, groups, raw, matchedSet, ignoredFilterGroup,
        ' · ≈ hybrid: ' + pass.length + ' semantic matches folded in');
  ...
  function entScan(text) {
    ...
    while ((m = R.domain.exec(t))) out.push({ k: 'domain', v: m[0].toLowerCase() });
    R.ip.lastIndex = 0;
    while ((m = R.ip.exec(t))) {
  ...
  function compileSkillFromRecipe(recipe, nm) {
    ...
    return semEnsureDocs().then(function () {
      ...
      /* per-part selection (each part keeps its own adaptive floor + the ≥2-passage "about" rule),
         then the union, deduped in part order */
      var chosen = [], seen = {}, perCount = [];
      parts.forEach(function (p, pi) {
        ...
        var sel = (strong.length >= 8 ? strong : order).slice(0, 150);
        perCount.push(sel.length);
        sel.forEach(function (u) {
          if (seen[u]) return; seen[u] = 1;
          var c = state.convs.get(u); if (c) chosen.push(c);
        });
      });
      chosen = chosen.slice(0, 150);
