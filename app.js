...
(function () {
  ...
  /* ---------- Index ---------- */
  var APP_VERSION = '1.58.0'; /* shown in the footer + the diagnostic report — bump per release */
  ...
  function semAugment(out, titleOnly, groups, searchableGroups, raw, matchedSet, ignoredFilterGroup) {
    var seq = ++semSeq;
    /* v1.58.0 — the one gate that matters: on a phone, ≈ never arms the model and never loads the
       50 MB of vectors. state.semOn is already forced false at init, so this is belt-and-braces —
       but it's the line that makes "no route drifts back in" true rather than hoped for. */
    if (!state.semOn || semDisabled || semDeskOnly()) return;
    ...
    /* v1.50.2 (live-caught: iOS crash-loop at BOOT): vectors are no longer preloaded at init on
       ...
       the whole site. */
    /* v1.57.2 (recorder-proven: 18:26:28 model load START → :29 ABRUPT boot): on iOS the wasm
       instantiate spike landing ON TOP of the ~50 MB resident vectors is what gets the tab killed.
       Arm the MODEL FIRST — the spike happens while memory is still light; vectors follow.
       DEAD SINCE v1.58.0 — the semDeskOnly() guard above returns before this can run. Kept (not
       ripped out) on purpose: the removal is its own build with its own smoke. Do not wire new
       behaviour into it — it is unreachable. */
    if (isIOS() && !semExtractor) {
      if (!semReRunArmed) {
        semReRunArmed = true;
        var rrLast0 = null;
        semLoad(function (s) {
          var m = /(\d+)\s*%/.exec(s); if (m) rrLast0 = 55 + Math.min(45, +m[1] * 0.45);
          showProgress('Semantic: ' + s, rrLast0);
        }).then(
  ...
  function openHelp() {
    ...
    if (hmMob) h.push('<h3>The map</h3>' +
      ...
      '<text class="hm-t" x="20" y="398">4 · semantic — opt-in, on a computer</text>' +
      ...
      '<text class="hm-m" x="20" y="462">phones: keyword search, stats, reader</text>' +
      ...
      '<text class="hm-t" x="36" y="288">4 · semantic — opt-in, on a computer</text>' +
      '<text class="hm-s" x="36" y="305">one local model download → your archive embedded as vectors, on-device</text>' +
      '<text class="hm-m" x="36" y="319">≈ hybrid search · model: e5 or Gemma · phones keep keyword search, stats, reader</text>' +
    ...
    h.push('<h3>Top bar \u2014 Stats, coach, me.skill, backup</h3>' +
      ...
      '<li><strong>Semantic search</strong> \u2014 (computer only) an opt-in AI model (self-hosted here) that also finds messages by MEANING, not just keywords; the \u2248 toggle by the search bar folds those matches into your results. It runs on a computer, not a phone: an iPhone can\u2019t hold the model and your vectors at once, and Safari reloads the tab mid-search \u2014 so rather than half-work, it\u2019s honestly not offered there. Your phone keeps keyword search over every word of every chat, plus browse, stats, entities and the reader.</li>' +
      '<li><strong>Semantic model</strong> \u2014 (computer only) switch the embedding model behind semantic search: <strong>e5</strong> (default) or <strong>EmbeddingGemma</strong>, which matches meaning noticeably more sharply. Switching to Gemma is a one-time ~190 MB self-hosted download plus a re-embed of your archive (roughly an hour \u2014 resumable, it continues where it stopped if you close the tab). Your e5 vectors are kept, so switching back is instant. Search, Compile and Suggest all run on whichever model is active — each has its own calibrated thresholds.</li>' +
  ...
  function openFaq() {
    ...
    h.push('<h3>Finding things again</h3><ul>' +
      ...
      '<li><strong>I only remember the idea, not the words.</strong> Turn on Semantic search once, then the <code>\u2248</code> toggle: \u201cthat chat about negotiating rent\u201d finds \u201cthe landlord wants 200 more\u201d \u2014 meaning, not keywords. Matches found this way are marked <code>\u2248</code>. (Computer only \u2014 it needs an AI model a phone can\u2019t hold.)</li>' +
    ...
    h.push('<h3>Privacy & data</h3><ul>' +
      ...
      '<li><strong>Can I use it on my phone?</strong> Yes, for everything except semantic: <strong>Download archive</strong> \u2192 one .zip \u2192 open colloquary on the phone and drop it there. You get keyword search across every word of every chat, browse, the reader, stats, entities, summaries and export. <strong>Semantic search (\u2248) and Compile are computer-only</strong> \u2014 see the next answer.</li>' +
      '<li><strong>Why isn\u2019t semantic search on my iPhone?</strong> Because it can\u2019t be honest there. Semantic needs the AI model and your vectors resident at the same time, on top of your archive \u2014 more than iOS grants a Safari tab, so it reloads the page mid-search. It was offered on phones for a while (import a .cvec made on your computer) and it genuinely half-worked: the import succeeds, then the first search reloads the tab. Adding the site to the Home Screen doesn\u2019t change it. A feature that fails at the moment you use it is worse than one that lives somewhere clear, so it now runs on computers only. The same goes for Compile / Suggest \u2014 they select passages by meaning, so they need the same model.</li>' +
    ...
    h.push('<h3>Honest limits</h3><ul>' +
      ...
      '<li><strong>Semantic search and Compile don\u2019t run on iPhones.</strong> iOS kills the tab under model memory pressure \u2014 measured, fought for eight releases, and honestly given up on. Everything else works there; the \u2248 and Compile surfaces live on your computer.</li></ul>');
  ...
  (function () {
    try {
      ...
      /* v1.57.4 — a visibilitychange can land AFTER pagehide (live-caught: a clean reload flagged
         ABRUPT); a clean exit counts if pagehide is anywhere in the last 3 lines */
      var l = blogRead(), last = l.slice(-3).join(' | ');
      if (l.length && !/pagehide/.test(last)) {
        blog('BOOT (' + (nav.type || '?') + ') — PREVIOUS SESSION ENDED ABRUPTLY (no pagehide → likely an iOS memory kill)');
        state.bootAbrupt = true; /* v1.57.2 — the abrupt-boot fact drives the query restore */
  ...
  /* v1.57.4 — delete ONE model's vectors (range) — storage hygiene for the model you don't run on
     this device (Eugen's gemma experiment left 33 MB of unused vectors adding origin pressure on
     iOS). Re-importing that model's .cvec restores them. */
  function semVecDeleteModel(mk) {
    return semVecDB().then(function (d) {
      return new Promise(function (res, rej) {
        var tx = d.transaction('vecs', 'readwrite');
        tx.objectStore('vecs').delete(semKeyRange(mk));
        tx.oncomplete = function () { res(); };
        tx.onerror = function () { rej(tx.error); };
      });
    });
  ...
  /* v1.58.0 — count vectors across EVERY registered model, not just the active one: a phone that
     followed the old "the phone follows the .cvec you import" rule can be holding both e5 and gemma
     rows, and the gemma ones would be invisible to a plain semVecCount(). */
  function semPhoneVecScan() {
    var ks = Object.keys(SEM_MODELS);
    return Promise.all(ks.map(function (k) { return semVecCount(SEM_MODELS[k].MODEL_KEY); })).then(function (cs) {
      var n = 0, bytes = 0, hit = [];
      for (var i = 0; i < ks.length; i++) {
        if (!cs[i]) continue;
        n += cs[i]; bytes += cs[i] * SEM_MODELS[ks[i]].DIMS * 4; hit.push(ks[i]);
      }
      return { n: n, mb: bytes / 1048576, keys: hit };
    });
  }
  /* v1.58.0 — vectors imported by an earlier version are now dead weight on a phone: nothing will
     ever read them, and 12–50 MB of origin storage is exactly the pressure that makes iOS purge the
     things you DO use. Offer them back — explicit confirm, never a silent delete of user data. */
  function semPhoneVecFree() {
    return semPhoneVecScan().then(function (r) {
      if (!r.n) { toast('No semantic vectors are stored on this phone.'); return; }
      var mb = r.mb.toFixed(0);
      if (!confirm(SEM_DESK_WHY + '\n\n' + r.n.toLocaleString() + ' vectors (~' + mb + ' MB) imported here by an earlier version are no longer used by anything.\n\nDelete them to free the space?\n\nYour archive, your searches and your pins are not affected — and you can import a .cvec again anytime if this ever changes.')) return;
      return Promise.all(r.keys.map(function (k) { return semVecDeleteModel(SEM_MODELS[k].MODEL_KEY); })).then(
        function () {
          semRun.docs = null; semRun.vecs = null;
          toast('Freed ~' + mb + ' MB (' + r.n.toLocaleString() + ' unused vectors). Your archive is untouched.');
        },
        function () { toast('Could not delete those vectors — try again.', true); }
      );
    });
  }
  ...
  /* v1.55.2 — is the model actually in Cache Storage? iOS purges it without persistent storage, and
     the re-download was invisible (Eugen: "left the tab, searched later — it downloads the llm again").
     Read-only; used by the self-test. */
  function semCacheInfo() {
    if (!(self.caches && caches.keys)) return Promise.resolve('model cached: ? (no Cache Storage API)');
            ...
            return Promise.all(reqs.filter(function (rq) { return /\.onnx(_data)?(\?|$)/.test(rq.url); })
              .map(function (rq) {
                return c.match(rq).then(function (res) {
                  hits.push((res && +res.headers.get('content-length')) || 0);
                });
              }));
          });
        });
      })).then(function () {
        if (!hits.length) return 'model cached: NO (next ≈ search downloads it)';
        var tot = hits.reduce(function (a, b) { return a + b; }, 0) / 1048576;
        return 'model cached: yes — ' + hits.length + ' file' + (hits.length > 1 ? 's' : '') + (tot ? ', ' + tot.toFixed(1) + ' MB' : '');
      });
  ...
  function semCacheUsableFiles() {
    /* v1.58.0 — semantic is desktop-only, so on a phone NO model file can ever load again: every
       cached .onnx is dead weight (Eugen's iPhone was holding 113 MB of it). Returning just the
       tokenizer makes the self-test's existing free-space offer reclaim the lot — the prune needed
       no new code, only an honest answer to "what can this device load?". */
    if (semDeskOnly()) return ['tokenizer.json'];
  ...
  /* ---------- v1.58.0 — SEMANTIC IS DESKTOP-ONLY (Eugen's call, 2026-07-15) ----------
     Eight versions fought iOS for semantic search (v1.50.0 boot crash-loop → v1.50.3 lazy arm →
     v1.52.x gemma NO → v1.55.2 cache visibility → v1.57.2 model-first arm + query restore →
     v1.57.4 vector delete), and the flight recorder ended the argument: it is not a bug, it is
     TOTAL RESIDENCY — the model session + ~50 MB of f32 vectors + the archive/index exceed what
     iOS grants a tab, so Safari kills it mid-search. The last hypothesis died on 2026-07-15:
     Add to Home Screen (which grants persistent storage far more readily) reloads exactly the
     same. int8 vectors would have bought ~37 MB against a ~10% overshoot — a delay, not a fix.
     So the phone stops pretending. What it keeps is the product: keyword search over every word,
     browse, reader, stats, entities, summary, dossier, pins, export. What moves to the computer
     is semantic — and the compile surface with it, because Compile / Suggest / custom lens all
     embed a phrase, which needs the model that doesn't fit.
     ONE predicate so no route can drift back in; desktop is untouched by construction. */
  function semDeskOnly() { return isIOS(); }
  /* the honest why, in one sentence — every blocked entry point says the same thing (a feature that
     "sometimes works" is worse than one that's honestly elsewhere) */
  var SEM_DESK_WHY = 'Semantic search runs on your computer. An iPhone can’t hold the model and your vectors at the same time — Safari reloads the tab mid-search, so offering it here would be a promise the phone can’t keep.';
  var SEM_DESK_OK = 'On this phone, search still reads every word of every chat — browse, stats, entities and the reader all work as usual.';
  ...
  function semEnsureDocs() {
    /* v1.58.0 — the backstop. Since v1.52.0 this was the ONE choke point enforcing "phones never
       EMBED"; it now enforces the stronger rule "phones never touch vectors at all". Every caller
       is guarded already — this line is what makes that a property of the code instead of a claim
       about the call graph, and it's where a future feature that forgets will land safely. */
    if (semDeskOnly()) return Promise.resolve(false);
  ...
  function openSuggest() {
    ...
    /* v1.58.0 — Suggest ranks topics by MEANING, which means embedding 15 seed phrases, which means
       the model: desktop-only since semantic is. Say where it lives, not "turn on X" (X is gone). */
    if (semDeskOnly()) { toast('Suggest skills runs on your computer — it ranks your topics by meaning, which needs the model an iPhone can’t hold. Your archive is already there if you imported it.', true); return; }
  ...
  function suggestNudge() {
    /* v1.58.0 — never nudge a phone toward a desktop-only surface (it can't fire there anyway: the
       nudge needs vectors, which phones no longer hold — but an onboarding toast that leads
       nowhere is the worst kind of dead end, so the rule is explicit rather than incidental) */
    if (suggestNudged || semDeskOnly()) return;
    semVecCount().then(function (n) {
      if (!n) return;
  ...
  function semAfterImport() {
    /* v1.58.0 (caught reviewing the v1.58.0 diff itself): this fires straight after an archive
       import — the heaviest memory moment a phone has — and the iOS branch below pulled ~50 MB of
       vectors into RAM via semRebuildFromStore. Every OTHER route was guarded and this one would
       still have loaded them. Semantic is desktop-only; a phone has nothing to rebuild here. */
    if (semDeskOnly()) return;
    semVecCount().then(function (n) {
  ...
  function init() {
    openDB().then(function (d) {
      ...
      /* v1.57.2 — abrupt-boot query restore: if iOS killed the tab mid-search (recorder-flagged) and
         the persisted query is fresh, put it back and re-run — the user returns mid-thought. Both
         state.query AND the input are set (the v1.47.1 rc-run rule). */
      if (state.bootAbrupt && state.convs.size) {
        try {
          var lq = JSON.parse(localStorage.getItem('chatalog-lastq') || 'null');
          if (lq && lq.q && Date.now() - lq.t < 600000) {
            state.query = lq.q; $('#search').value = lq.q; syncClear();
            blog('query restored after abrupt end');
            toast('Restored your search after the reload.');
          }
        } catch (e) { /* best effort */ }
      }
      if (state.convs.size) {
      ...
      /* v1.58.0 — phones: semantic is desktop-only, so the ≈ chip never appears, no vectors ever
         load, and the eviction diagnostics below are skipped (they'd now be noise about a feature
         this device doesn't have — the "your vectors were removed" toast was true and useless).
         Whatever an earlier version imported here is offered back as free space instead. */
      if (semDeskOnly()) {
        if (state.semOn) { state.semOn = false; setMeta('semOn', false); }
        semPhoneVecScan().then(function (r) {
          if (!r.n) return;
          toast('Semantic search now runs on your computer — the ' + r.n.toLocaleString() + ' vectors stored on this phone (~' + r.mb.toFixed(0) + ' MB) are unused. TAP THIS MESSAGE to free the space.', true);
          state._toastAction = semPhoneVecFree; /* armed AFTER the toast (the stale-action rule) */
        });
        return;
      }
    ...
    /* Save the vector store to a .cvec file — a backup, or a way to skip re-embedding on a second
       computer. (Until v1.58.0 this file's purpose was the phone; that flow is retired.) */
    function downloadEmbeddings() {
      /* v1.58.0 (second diff-review catch): on a phone with un-cleaned vectors this read the WHOLE
         store into memory via semExportBlob → semVecLoadAll — the exact 50 MB load the release
         exists to prevent, reachable from a menu item nobody guarded. */
      if (semDeskOnly()) { toast(SEM_DESK_WHY + ' There are no embeddings here to save.', true); return; }
      semVecCount().then(function (n) {
        ...
        semExportBlob().then(function (r) {
          ...
          setTimeout(function () { URL.revokeObjectURL(a.href); a.remove(); }, 1000);
          /* v1.58.0: the old copy sent this file to a phone — the flow semantic-on-mobile was built
             around, and the one we retired. It's now a backup / move-to-another-computer file. */
          toast(r.count + ' embeddings saved (' + (r.blob.size / 1048576).toFixed(0) + ' MB) — a backup, or import it on another computer to skip re-embedding there.');
    ...
    function importEmbeddings(file) {
      /* v1.58.0 — second lock on the same door: the menu action is guarded, but #vec-input is a DOM
         node any future code (or a stale open dialog) could fire. Vectors must not land on a phone. */
      if (semDeskOnly()) { toast(SEM_DESK_WHY + ' Embeddings imported here would sit unused.', true); return; }
    ...
    var TOPNAV_ACTS = {
      ...
      compile: function () {
        ...
        /* v1.58.0 — desktop-only for the same reason as ≈: selecting passages by meaning embeds the
           phrase, which loads the model, which is the exact thing that kills the tab here. */
        if (semDeskOnly()) { toast('Compile a topic runs on your computer — it selects passages by meaning, which needs the model an iPhone can’t hold. ' + SEM_DESK_OK, true); return; }
      ...
      compileskill: function () {
        ...
        /* v1.58.0 — desktop-only (the lens embed needs the model). Compiling a skill is a
           sit-at-a-desk act anyway — the .skill it produces gets installed on a computer. */
        if (semDeskOnly()) { toast('Compile a skill runs on your computer — it selects the topic by meaning, which needs the model an iPhone can’t hold. Open colloquary there with the same archive.', true); return; }
        semVecCount().then(function (n) {
      ...
      /* v1.58.0 — importing a .cvec here would succeed and then help nothing: the vectors would sit
         unread while adding the storage pressure that makes iOS purge what you do use. */
      semimport: function () {
        if (semDeskOnly()) { toast(SEM_DESK_WHY + ' Embeddings imported here would sit unused.', true); return; }
        $('#vec-input').click();
      },
      ...
      semmodel: function () {
        ...
        /* v1.58.0 — nothing to choose between when no model runs here. (This replaces the v1.52.0
           iOS escape hatch: its whole job was getting a phone off gemma and back onto e5 so
           semantic worked again — a question that no longer exists.) */
        if (semDeskOnly()) { toast(SEM_DESK_WHY, true); return; }
      ...
      semtest: function () {
        ...
        var pP = (navigator.storage && navigator.storage.persisted)
          ...
          : Promise.resolve('?');
        var cP = semCacheInfo(); /* v1.55.2 — make the invisible re-download visible */
        Promise.all(checks.map(function (c) {
          ...
          Promise.all([pP, cP]).then(function (pc) {
            ...
            /* v1.57.4 — same idea one layer down: vectors of a model this device isn't running */
            Object.keys(SEM_MODELS).forEach(function (mk2) {
              var M = SEM_MODELS[mk2];
              if (M.MODEL_KEY === SEM.MODEL_KEY) return; /* the active model stays */
              semVecCount(M.MODEL_KEY).then(function (n) {
                if (!n) return;
                var mb = (n * M.DIMS * 4 / 1048576).toFixed(0);
                if (confirm(n.toLocaleString() + ' ' + M.LABEL + ' vectors are stored on this device (~' + mb + ' MB) but the active model is ' + SEM.LABEL + '. Delete them to lighten storage? Importing that model’s .cvec restores them anytime.')) {
                  semVecDeleteModel(M.MODEL_KEY).then(
                    function () { toast('Deleted ' + n.toLocaleString() + ' ' + M.LABEL + ' vectors (~' + mb + ' MB).'); },
                    function () { toast('Could not delete those vectors — try again.', true); }
                  );
                }
              });
      ...
      semantic: function () {
        ...
        /* v1.58.0 — the v1.50.0 branch that used to live here TAUGHT the desktop → .cvec → phone
           flow. The teaching worked; the flow didn't. The phone imports the vectors fine and then
           Safari kills the tab on the first ≈ search, because the model + the vectors + the archive
           don't fit at once. So this action now states the truth instead of routing the user into a
           dead end — and hands back the storage the old advice left behind. */
        if (semDeskOnly()) {
          semPhoneVecScan().then(function (r) {
            if (r.n) { semPhoneVecFree(); return; } /* its confirm carries the why + frees the space */
            alert(SEM_DESK_WHY + '\n\nOn your computer:\n1. open colloquary and import your archive\n2. run Semantic search (embeds once, locally)\n3. search by meaning — and compile skills from it\n\n' + SEM_DESK_OK);
          });
          return;
        }
        semVecCount().then(function (n) {
      ...
      clear: function () {
        ...
        clearAll().then(function () {
          ...
          try { localStorage.removeItem('chatalog-bootlog'); localStorage.removeItem('chatalog-lastq'); } catch (e) { /* v1.57.1/.2 — recorder + saved query go too */ }
          state.semOn = false; setMeta('semOn', false);
    ...
    $('#search').addEventListener('input', function (e) {
      ...
      /* v1.57.2 — sync-persist the in-flight query: an iOS memory kill mid-search reloads the page
         with an empty box (Eugen: "the search typed disappears"); the abrupt-boot restore reads this */
      try { localStorage.setItem('chatalog-lastq', JSON.stringify({ q: state.query, t: Date.now() })); } catch (err) { /* best effort */ }
      syncClear();
