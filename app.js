...
(function () {
  ...
  /* ---------- Index ---------- */
  var APP_VERSION = '1.57.4'; /* shown in the footer + the diagnostic report — bump per release */
  ...
  function openFaq() {
    ...
    h.push('<h3>Privacy & data</h3><ul>' +
      ...
      '<li><strong>Can I use it on my phone?</strong> Yes: <strong>Download archive</strong> \u2192 one .zip \u2192 open colloquary on the phone and drop it there. For semantic on the phone: embed on desktop \u2192 <strong>Download embeddings</strong> (.cvec) \u2192 <strong>Import embeddings</strong> on the phone. Use an <strong>e5</strong> .cvec \u2014 e5 (~118 MB model) is the one that runs on phones (Gemma\u2019s model doesn\u2019t load on iOS today; the app says so and offers the way back). One iOS habit to know: when Safari reclaims a background tab it may also clear the downloaded model, so the next \u2248 search downloads it again \u2014 colloquary asks iOS for persistent storage (the self-test shows whether it was granted), and adding the site to your Home Screen can help iOS keep it.</li>' +
